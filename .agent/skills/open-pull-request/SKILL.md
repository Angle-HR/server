...
description: Use this skill when the user wants to open a PR, create a pull request, run gh pr create, or submit the active branch for review. Opens a GitHub pull request for the current branch against `main`, using the GitHub CLI when available and preferring a description from `pr_template/pr-<branch>.md` when present.
---
...
3. **If missing** — Run **`make pr-description`** (see the **generate-pr-description** skill), fill the draft, then use the resulting file—or fall back to **`gh pr create --fill`** using commits, or a short body summarizing `git log main..HEAD` and `git diff main --stat`.
...
If there is no suitable body file yet, run the **generate-pr-description** skill first (or in sequence), then open the PR with **`--body-file`** pointing at the completed draft.
